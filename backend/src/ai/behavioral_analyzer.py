        ...
        if hist_urls and curr_domains:
            ...
            if new_domains:
                evidence.append(EvidenceItem(
                    ...
                    explanation=f"Sender is using new URL domains ({', '.join(new_domains)}) not previously observed for them."
                ))

        # 2. Authentication behavior changes
        hist_auth = history.get("auth_summary", [])
        
        curr_auth_str = f"SPF:{current_auth.get('spf','none')} DKIM:{current_auth.get('dkim','none')} DMARC:{current_auth.get('dmarc','none')}"
