...
Logged actions:
    email_viewed, verdict_overridden, feedback_submitted,
    case_created, case_closed, ioc_marked_trusted, ioc_marked_suspicious,
    analyst_verdict_submitted, case_note_added
"""
...
_FORBIDDEN_FIELDS = {
    "access_token", "refresh_token", "token", "session_id", "cookie",
    "password", "secret", "credentials", "private_key", "api_key"
}
...
class AuditLog:
    ...
    def log(
        ...
        Args:
            action: Action name (e.g., 'feedback_submitted')
        ...
        try:
            with get_db() as conn:
                conn.execute(
                    "INSERT INTO audit_log (action, actor, details, timestamp) VALUES (?, ?, ?, ?)",
                    (action, actor, json.dumps(safe_details), now)
                )
            return True
            ...
            with get_db() as conn:
                rows = conn.execute(
                    """SELECT * FROM audit_log
                       WHERE details LIKE ? ORDER BY timestamp DESC""",
                    (f'%"{message_id}"%',)
                ).fetchall()
                results = rows_to_list(rows)
                for r in results:
